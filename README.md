...
This project uses a PostgreSQL database to persist data. For local development, a docker-compose file is included to spin up a new instance. This instance runs on port 5433 instead of the default of 5432 to avoid conflicts with other PostgreSQL instances.

Knex migrations are used to quickly build out the database. To reset the database, just run `npm run db:rollback` followed by `npm run db:migrate`. This will drop and re-create all tables.

1. Install npm packages: `npm install`
2. Run database instance: `docker-compose up`
3. Run knex migrations: `npm run db:migrate`
